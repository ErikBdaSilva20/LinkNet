...
export default function PublicProfile() {
  const { handle, pageSlug } = useParams<{ handle: string; pageSlug?: string }>();
  const {
    profile,
    page,
    links,
    ...
    error,
    trackClick,
  } = usePublicProfile(handle, pageSlug);
      ...
      <div className="min-h-screen w-full flex items-center justify-center p-4 bg-background">
        <div className="text-center space-y-4">
          ...
          <p className="text-muted-foreground">
            O usuário @{handle}{pageSlug ? `/${pageSlug}` : ""} não existe ou não está público.
          </p>
        </div>
      </div>
    );
  ...
  const pageTitle = page.title || profile.display_name || `@${handle}`;
  const pageDescription = page.description || profile.bio || "";
  const pageImage = page.og_image_url || profile.avatar_url;
  ...
  return (
    <>
      ...
      {/* Integration Scripts (GA4, Meta Pixel, Custom HTML) */}
      <IntegrationScripts
      ...
      >
        ...
        >
          ...
          <div className="text-center space-y-4">
            ...
            >
              {profile.avatar_url ? (
                ...
                profile.display_name?.charAt(0) || handle?.charAt(0) || "U"
              )}
            </div>
          ...
          {/* Lead Capture Form */}
          {(page as Page & { lead_form_enabled?: boolean })?.lead_form_enabled && (
            <LeadCaptureForm
              pageId={page.id}
              title={(page as Page & { lead_form_title?: string })?.lead_form_title || "Fique por dentro"}
              description={(page as Page & { lead_form_description?: string })?.lead_form_description || "Cadastre seu e-mail para receber novidades"}
              accentColor={accentColor}
              textColor={textColor}
              buttonRadius={buttonRadius}
              fields={parseFormFields((page as Page & { lead_form_fields?: unknown })?.lead_form_fields)}
            />
          )}
